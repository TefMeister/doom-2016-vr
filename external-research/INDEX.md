...
**Last `/gr` pass: 2026-09-04 (estate sweep, second pass today) — CHECK-IN, targeted** (today's `ringcam` note §10, INDEX; three first-party spec reads)**.** Inbox empty. **One new topic, answering a contradiction the modding lane flagged hours earlier.** A flush count on host-coherent memory is legal and carries no information about the update route, so §6g and §10 never conflicted and one of the two proposed `ringcam` redesigns rests on a non-signal. Names the one static field read that would make the contradiction real if it went the other way. My 2026-09-02 eye-field drop is still unread in `engine-research/inbox/` and was left alone.

_Previous: **Last `/gr` pass: 2026-09-04 (estate sweep) — CHECK-IN** (board OPEN block, re-audited 2026-09-03 by `/pd`, + INDEX)**.** Inbox empty; my 2026-09-02 eye-field drop is still unread in `engine-research/inbox/` and was left alone. **Nothing new, and not searched** — the one `[PD]` row (eye field on the view object) is what that pending drop addresses; the rest need a launch, a reboot, or a user decision._
_Previous: **Last `/gr` pass: 2026-09-03 (estate sweep) — CHECK-IN** (board OPEN block + INDEX)**.** Inbox empty; my 2026-09-02 eye-field drop is still unread in `engine-research/inbox/` and was left alone. **Nothing new, and little left to search.** The one `[PD]` row — mine the reflection database for an eye field on the view object — is what that pending drop already answers; the remaining rows need a `multiView_60Hz` run, a reboot for the ASLR re-check, or a user decision on `DOOMLegacyMod`. The cvar space in particular is exhausted: the 2026-09-02 pass downloaded the whole 711 KB / 6,572-entry file rather than fetching a page, so "is there a stereo MODE cvar" is settled negative on a complete read, not a truncated one._
...
| --- | --- | --- | --- |
| 2026-09-04 | [A flush count on coherent memory is not evidence of an update route](topics/2026-09-04-a-flush-count-on-coherent-memory-is-not-evidence-of-an-update-route.md) | 🆕 new | **Resolves the contradiction today's `ringcam` note asked to be resolved before either redesign is built.** The Vulkan spec says flush/invalidate are **"not needed"** on `HOST_COHERENT` memory, and nothing forbids them `[reported 2026-09-04, first-party]` — so §6g's "coherent, therefore not the update route" and §10's `flushes=27462` on region 2 are **consistent**, explained by an engine that flushes unconditionally. Neither claim needs retracting. **The consequence:** a flush count carries no information about the update route, so redesign option 2 (reuse the per-mapping flush path) rests on a non-signal; option 1 (value-locate the region, bounded scan, existing column-3 guard) is the sound one and `findvec` already evidences it. ⚠️ Assumes region 2 really is coherent — one static field read of its `VkMemoryPropertyFlags` decides, and a non-coherent answer makes the contradiction real. |
